...
int AudioEncodedFrameFileWriter::initialize(const char *dstPath, AVCodecParameters *codecpar) {
...
void AudioEncodedFrameFileWriter::processAudioPacket(AVPacket *audio_packet, AVStream *src_stream) {
    //输入流和输出流的时间基可能不同，因此要根据时间基的不同对时间戳pts进行转换
    audio_packet->pts = av_rescale_q(audio_packet->pts,
