...
public:
    ...
    int close();
    int flush();
    int setVideoFrameProcessing(std::shared_ptr<IVideoFrameProcessing> processing);
    AVCodecContext *getVideoCodecContext();

public:
    void processVideoPacket(AVPacket *video_packet, AVStream *src_stream) override;

private:
    int decodeLocked(AVCodecContext *avCodecContext, AVFrame *avFrame, AVPacket *pkt);
