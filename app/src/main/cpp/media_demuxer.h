...
public:
    ...
    virtual void processVideoPacket(AVPacket *video_packet, AVStream *src_stream) = 0;
};

class IAudioPacketProcessing {
public:
    virtual ~IAudioPacketProcessing();
    virtual void processAudioPacket(AVPacket *audio_packet, AVStream *src_stream) = 0;
};
