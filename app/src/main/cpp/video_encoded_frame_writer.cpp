...
int VideoEncodedFrameWriter::initialize(const char *dstPath, AVCodecParameters *codecpar) {
    ...
    if ((ret = avio_open(&format_context_->pb, file_path_.c_str(), AVIO_FLAG_WRITE)) < 0) {
...
void VideoEncodedFrameWriter::processVideoPacket(AVPacket *video_packet, AVStream *src_stream) {
    //输入流和输出流的时间基可能不同，因此要根据时间基的不同对时间戳pts进行转换
    video_packet->pts = av_rescale_q(video_packet->pts, src_stream->time_base, out_stream_->time_base);
