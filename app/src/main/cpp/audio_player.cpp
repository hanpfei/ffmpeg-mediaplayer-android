...
void AudioPlayer::ProcessSLCallback(SLAndroidSimpleBufferQueueItf bq) {
  ...
  sample_buf *buf = &buffer_;
  if (data_callback_) {
    data_callback_(data_ctx_, sampleInfo_.sampleRate_,
                   sampleInfo_.framesPerBuf_,
                   sampleInfo_.channels_, sampleInfo_.pcmFormat_, buf->buf_);
...
AudioPlayer::AudioPlayer(SampleFormat *sampleFormat, SLEngineItf slEngine)
  ...
  buffer_.cap_ = (format_pcm.containerSize >> 3) * format_pcm.numChannels *
                 sampleInfo_.framesPerBuf_;
  buffer_.buf_ = new uint8_t[buffer_.cap_];
  memset(buffer_.buf_, 0, buffer_.cap_);
  buffer_.size_ = buffer_.cap_;
...
SLresult AudioPlayer::Start(void) {
  ...
  result =
      (*playBufferQueueItf_)
          ->Enqueue(playBufferQueueItf_, buffer_.buf_, buffer_.size_);
  SLASSERT(result);
...
void AudioPlayer::RegisterDataCallback(ENGINE_PLAYBACK_DATA_CALLBACK cb, void *data_ctx) {
  data_callback_ = cb;
  data_ctx_ = data_ctx;
}
