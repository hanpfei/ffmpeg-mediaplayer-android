...
int MediaDemuxer::open(const char *url) {
    ...
    //video_index
    video_index_ = av_find_best_stream(format_context_, AVMEDIA_TYPE_VIDEO, -1, -1, nullptr, 0);
    if (video_index_ < 0) {
...
int MediaDemuxer::seek(float position) {
    std::lock_guard<std::mutex> _l(lock_);
    if (video_stream_) {
        auto target_video_frame = static_cast<int64_t>(video_stream_->duration * position);
        LOGW("Video duration %" PRId64 ", target video position %" PRId64 "",
             video_stream_->duration, target_video_frame);
        avformat_seek_file(format_context_, video_index_, 0, target_video_frame, target_video_frame, AVSEEK_FLAG_FRAME);
    }
    if (audio_stream_) {
        auto target_audio_frame = static_cast<int64_t>(audio_stream_->duration * position);
        LOGW("Audio duration %" PRId64 ", target audio duration %" PRId64 "",
             audio_stream_->duration, target_audio_frame);
        avformat_seek_file(format_context_, audio_index_, 0, target_audio_frame, target_audio_frame, AVSEEK_FLAG_FRAME);
    }
...
int MediaDemuxer::setVideoPacketProcessing(std::shared_ptr<IVideoPacketProcessing> processing)  {
...
int MediaDemuxer::setAudioPacketProcessing(std::shared_ptr<IAudioPacketProcessing> processing) {
