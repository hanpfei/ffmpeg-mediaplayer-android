...
int AudioFrameWriter::resample(uint8_t *audio_data, AVFrame *frame, FILE *outfile_resampled) {
    ...
    if (!swr_ctx_) {
        ...
        dest_samples_per_frame_ =
            av_rescale_rnd(samples_per_frame, dest_sample_rate_, frame->sample_rate, AV_ROUND_UP);
        ...
        ret = av_samples_alloc_array_and_samples(&sw_dst_data_, &dst_linesize, dst_nb_channels,
                                                 dest_samples_per_frame_, dest_sample_format_, 0);
        if (ret < 0) {
    ...
    while (sample_position <= (left_samples_ - samples_per_frame)) {
        ...
        int dest_samples_per_frame = av_rescale_rnd(delay_samples +
            samples_per_frame, dest_sample_rate_, frame->sample_rate, AV_ROUND_UP);
